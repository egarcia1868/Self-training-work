...
function Board() {
  ...
  this.killChecker = function(position) {
    ...
    // console.log(this.checkers.indexOf(selectChecker));
...
function Game() {
  ...
  this.moveChecker = function(start, end) {
    ...
    if ((end === `${Number(start)+11}` || end === `${Number(start)+9}` ||
      end === `${Number(start)-11}` || end === `${Number(start)-9}`) { // && (end[1] !== 8 && end[1] !== 9)) {
    //checks to see if its a legal move
      this.board.grid[end[0]][end[1]] = checker;
      console.log('start: ', start,'end: ', end, 'start+11: ', `${Number(start)+11}`, 'end[1]:', end[1]);
